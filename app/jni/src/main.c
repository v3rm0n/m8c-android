...
void close_serial_port(libusb_device_handle *devh) {
    SDL_Log("Closing serial port");
    disconnect(devh);
...
int main(int argc, char *argv[]) {
    //sp_set_debug_handler(debug_handler);
    ...
    struct libusb_device_handle *port = NULL;
    ...
    do {
        if (port == NULL) {
            port = init_serial(fileDescriptor, 1);
        }
        if (port != NULL) {
            int result;
        ...
        if (conf.wait_for_device) {
            ...
            while (run == WAIT_FOR_DEVICE) {
                ...
                if (!port && (SDL_GetTicks() - ticks_poll_device > 1000)) {
                    ...
                    port = init_serial(fileDescriptor, 0);
        ...
        while (run == RUN) {
            ...
            while (1) {
                // read serial port
                int bytes_read =
                        nonblocking_read(port, serial_buf, serial_read_size);
                if (bytes_read < 0) {
